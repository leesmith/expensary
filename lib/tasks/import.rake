namespace :import do
  ...
  task :apple, [ :filepath ] => :environment do |t, args|
    ...
    CSV.foreach(filepath, headers: true) do |row|
      ...
      # 04/08/2024,04/08/2024,"Kroger","Kroger","Grocery","Purchase","39.34","Tom Jones"

      unless row[4] == "Payment" && row[5] == "Payment" # Ignore payments
  ...
  task :delta_amex, [ :filepath ] => :environment do |t, args|
    ...
    CSV.foreach(filepath, headers: true) do |row|
      ...
      # 07/02/2025,Kroger,TOM JONES,-01023,30.45

      unless row[1] == "MOBILE PAYMENT - THANK YOU" # Ignore payments
        trans << ({
          account_id: account.id,
          tran_date: Date.strptime(row[0], "%m/%d/%Y"),
          description: row[1],
          amount: row[4].gsub("-", ""), # Amounts are always positive values
  ...
  task :chase_visa, [ :filepath ] => :environment do |t, args|
    ...
    CSV.foreach(filepath, headers: true) do |row|
      ...
      # 08/24/2023,08/25/2023,Kroger,Groceries,Sale,-13.00,

      unless row[4] == "Payment" # Ignore payments
        trans << ({
          account_id: account.id,
          tran_date: Date.strptime(row[0], "%m/%d/%Y"),
          description: row[2],
  ...
  task :regions_visa, [ :filepath ] => :environment do |t, args|
    ...
    CSV.foreach(filepath, headers: true) do |row|
      ...
      # "CASH REWARDS VISA SIGNATURE * 3970","02/15/2024","02/16/2024","*3970","Kroger","-54.9300","","KROGER"
